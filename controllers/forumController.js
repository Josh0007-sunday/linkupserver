...
const createForum = async (req, res) => {
  try {
    ...
    const forum = new Forum({
      ...
      creator: creatorId,
      creatorName: creator.name,
      creatorImg: creator.img || DEFAULT_USER_IMAGE,
      creatorStatus: creator.status || "Active",
      isPublic,
...
const joinForum = async (req, res) => {
  try {
    ...
    // Notify all clients in the forum about the new attendee
    const io = req.app.get('io');
    io.to(forumId).emit('userJoined', {
      user: {
        id: userId,
        name: user.name,
        img: user.img || DEFAULT_USER_IMAGE,
...
const sendMessage = async (req, res) => {
  try {
    ...
    // Get the Socket.io instance and emit the new message
    const io = req.app.get('io');
    io.to(forumId).emit('newMessage', newMessage);
...
const deleteForum = async (req, res) => {
  try {
    ...
    // Notify all clients that the forum was deleted
    const io = req.app.get('io');
    io.to(forumId).emit('forumDeleted', { forumId });
